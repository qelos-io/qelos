...
export const updateBlueprintCalling = {
  type: 'function',
  name: "updateBlueprintEntity",
  description: "Update an existing blueprint entity",
  function: {
    name: 'updateBlueprintEntity',
    description: 'Update an existing blueprint entity by identifier',
    parameters: {
      type: 'object',
      properties: {
        identifier: { type: 'string', description: 'Unique identifier for the blueprint / data model. use the name and remove spaces and special characters.' },
  ...
  handler: async (req, payload: Partial<IBlueprint & { fields: Record<string, any> | Array<any> }> = {}) => {
    ...
    const tenant = req.headers.tenant;
    const blueprint = await updateBlueprint(tenant, payload.identifier, payload);
