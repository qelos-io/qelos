...
export const createBlueprintCalling =  {
    ...
    parameters: {
      type: 'object',
      properties: {
        identifier: { type: 'string', description: 'Unique identifier for the blueprint. use the name and remove spaces and special characters.' },
        name: { 
          type: 'string', 
        ...
        entityIdentifierMechanism: { 
          type: 'string', 
          enum: ['objectid', 'guid'],
          description: 'Mechanism used for entity identification (choose objectid as default)' 
        },
        permissions: {
          type: 'array',
          items: {
        ...
        dispatchers: {
          type: 'object',
          properties: {
            create: { type: 'boolean', description: 'Whether create dispatchers are enabled' },
            update: { type: 'boolean', description: 'Whether update dispatchers are enabled' },
            delete: { type: 'boolean', description: 'Whether delete dispatchers are enabled' }
          },
          required: ['create', 'update', 'delete'],
          description: 'Dispatcher settings for the blueprint'
        },
      },
      required: ['identifier', 'name', 'entityIdentifierMechanism', 'permissions', 'permissionScope', 'properties', 'updateMapping', 'relations', 'dispatchers']
    },
    function: {
      name: 'createBlueprint',
      description: 'Create a new blueprint for the application. Returns the created blueprint.',
     },
     handler: async (req, payload: Partial<IBlueprint> = {}) => {
      const tenant = req.headers.tenant;
