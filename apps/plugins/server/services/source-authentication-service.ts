...
export async function storeEncryptedSourceAuthentication(tenant: string, kind: IntegrationSourceKind, authentication: any = {}, authId = uniqid()) {
  ...
  if (kind === IntegrationSourceKind.N8n || kind === IntegrationSourceKind.Sumit || kind === IntegrationSourceKind.Paddle) {
  ...
  if (kind === IntegrationSourceKind.AWS) {
    ...
    await setSecret(tenant, `integration-source-${kind}-${authId}`, { secretAccessKey });
    return authId;
  }

  if (kind === IntegrationSourceKind.Cloudflare) {
    const { apiToken } = authentication;
    await setSecret(tenant, `integration-source-${kind}-${authId}`, { apiToken });
    return authId;
  }
