...
import { IGoogleSource, IntegrationSourceKind } from '@qelos/global-types';
import { watch } from 'vue';
import { ref, defineProps, defineEmits } from 'vue';

const props = defineProps({
  modelValue: {
    type: Object as () => IGoogleSource,
  },
});

const emit = defineEmits(['update:modelValue', 'submit', 'close']);

const formModel = ref({ ...props.modelValue, kind: IntegrationSourceKind.Google });
...
watch(() => props.modelValue, (newValue) => {
  formModel.value = { ...newValue, kind: IntegrationSourceKind.Google };
