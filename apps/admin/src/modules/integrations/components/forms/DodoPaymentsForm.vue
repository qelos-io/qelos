<template>
  <el-form :model="formModel" :rules="rules" ref="formRef" @submit.prevent="submitForm" label-position="top">
    <FormInput
      ...
      required
      placeholder="Enter a descriptive name for this DodoPayments Connection"
    />
    ...
    >
      <el-input
        v-model="tokenInput"
        placeholder="Enter your DodoPayments API Key"
        type="password"
        show-password
        size="large"
        dir="ltr"
      ...
      <div v-if="showTokenHelp" class="token-help-section">
        <h4>How to get your DodoPayments API Key:</h4>
        <ol>
          <li>Go to <el-link type="primary" href="https://docs.dodopayments.com/" target="_blank" :underline="false">https://docs.dodopayments.com/</el-link></li>
          <li>Log in to your DodoPayments dashboard</li>
          <li>Navigate to <strong>Settings</strong> &gt; <strong>API Keys</strong></li>
          <li>Generate or copy your <strong>API Key</strong></li>
          <li>Choose the matching environment (Test for testing, Live for production)</li>
        </ol>
        <div class="token-warning">
          <el-icon><Warning /></el-icon> Your API Key gives access to DodoPayments services. Keep it secure and never share it publicly.
        </div>
      </div>
    </el-form-item>
...
import { IDodoPaymentsSource } from '@qelos/global-types';
...
const props = defineProps({
  modelValue: {
    type: Object as () => IDodoPaymentsSource,
  },
});
...
const validateForm = async () => {
  ...
  try {
    ...
    if (tokenInput.value && tokenInput.value.length < 10) {
      ElMessage.error('Please enter a valid DodoPayments API Key');
      return false;
    }
