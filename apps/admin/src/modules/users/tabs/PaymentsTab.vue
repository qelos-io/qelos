  ...
  <div class="payments-tab">
      ...
      <template #header>
        <div class="card-header">
          <div>
            ...
            <p class="subtitle">{{ $t('Select the plan that works best for you') }}</p>
          </div>
          <el-button
    ...
    >
      ...
      <p class="cancel-note" v-if="subscription?.currentPeriodEnd">
        {{ $t('Your access will continue until {date}.', { date: new Date(subscription.currentPeriodEnd).toLocaleDateString() }) }}
      </p>
      <template #footer>
...
const entityType = ref<BillableEntityType>('user')
const entityId = computed(() => props.userId || props.user?._id || '')
...
async function handleCheckout() {
  const result = await initiateCheckout({
    ...
    billableEntityId: entityId.value,
...
async function handleCancel() {
  ...
  if (result) {
    ...
    ElNotification.success({ title: t('Canceled'), message: t('Your subscription has been canceled.') })
    refreshSubscription()
  } else {
