...
describe('Push Command - Hard Flag Behavior', { concurrency: 1 }, () => {
  ...
  describe('Helper functions behavior', () => {
    it('should filter local files correctly by type', () => {
      ...
      assert.ok(result.output.includes('components: 1'));
      assert.ok(result.output.includes('blueprints: 1'));
      assert.ok(result.output.includes('plugins: 1'));
      assert.ok(result.output.includes('integrations: 1'));
    });
    ...
    it('should extract identifiers from filenames correctly', () => {
      ...
      const result = runHardPushScript(script);
      assert.ok(result.success);
      assert.ok(result.output.includes('component-id: my-component'));
      assert.ok(result.output.includes('blueprint-id: my-bp'));
      assert.ok(result.output.includes('plugin-id: my-plugin'));
      assert.ok(result.output.includes('integration-id: my-int'));
    });
  });
  ...
  describe('Hard push workflow simulation', () => {
    it('should simulate the workflow of finding resources to remove', () => {
      ...
      const result = runHardPushScript(script);
      assert.ok(result.success);
      assert.ok(result.output.includes('to-remove-components: 2'));
      assert.ok(result.output.includes('to-remove-blueprints: 1'));
      assert.ok(result.output.includes('components-to-remove: remote-comp-1,remote-comp-2'));
      assert.ok(result.output.includes('blueprints-to-remove: remote-bp-1'));
    });
  });
});
