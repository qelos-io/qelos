...
export function registerMiddlewareTests(
  expressModule: typeof express,
  label: string,
): void {
  test(`[${label}] createQelosMiddleware attaches user, workspace, and workspaces`, async () => {
  ...
  test(`[${label}] forwards inbound cookie and authorization to /api/me`, async () => {
  ...
  test(`[${label}] Set-Cookie from /api/me is forwarded with Domain rewritten to inbound host`, async () => {
    const mockFetch = async (input: RequestInfo) => {
      const url = String(input);
      if (url.includes('/api/me')) {
        return jsonResponse(200, minimalUser, [
  ...
  test(`[${label}] anonymous request leaves user null when requireAuth is false`, async () => {
    const mockFetch = async (input: RequestInfo) => {
      const url = String(input);
      if (url.includes('/api/me')) {
  ...
  test(`[${label}] requireAuth returns 401 when /api/me does not identify a user`, async () => {
    const app = expressModule();
    app.use(
      createQelosMiddleware({
        config: {
  ...
  test(`[${label}] skipPaths bypasses middleware`, async () => {
  ...
  test(`[${label}] resolveWorkspace can pick a workspace`, async () => {
  ...
  test(`[${label}] workspace defaults to user.workspace, not workspaces[0]`, async () => {
    const wsB: IWorkspace = { _id: 'ws-2', name: 'Second', labels: [] };
    const mockFetch = async (input: RequestInfo) => {
      const url = String(input);
      if (url.includes('/api/me')) {
  ...
  test(`[${label}] requireUser responds 401 when mounted without user`, async () => {
    const app = expressModule();
    app.use(
      createQelosMiddleware({
        config: {
  ...
  test(`[${label}] apiToken mode wires the SDK with x-api-key for downstream calls`, async () => {
  ...
  test(`[${label}] skipPaths matches multiple prefixes`, async () => {
    const app = expressModule();
    app.use(
      createQelosMiddleware({
        config: {
