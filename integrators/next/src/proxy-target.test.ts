...
describe('resolveQelosProxyTarget', () => {
  ...
  it('uses NEXT_QELOS_PROXY_TARGET when set, even if other env vars and appUrl exist', () => {
    process.env.NEXT_QELOS_PROXY_TARGET = 'https://from-next-env.example';
    process.env.QELOS_IP = 'https://from-qelos-ip.example';
    process.env.QELOS_API_IP = 'https://from-qelos-api-ip.example';
    const config: QelosNextConfig = {
      appUrl: 'https://from-app-url.example',
    };
    assert.equal(
      resolveQelosProxyTarget(config),
      'https://from-next-env.example',
    );
  });

  it('falls back to QELOS_IP when NEXT_QELOS_PROXY_TARGET is unset', () => {
    process.env.QELOS_IP = 'https://from-qelos-ip.example';
    process.env.QELOS_API_IP = 'https://from-qelos-api-ip.example';
    const config: QelosNextConfig = {
      appUrl: 'https://from-app-url.example',
  ...
  it('treats whitespace-only values as unset', () => {
    process.env.NEXT_QELOS_PROXY_TARGET = '   ';
