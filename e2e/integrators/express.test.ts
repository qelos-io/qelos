 ...
 *
 * One Express server hosts three middleware mounts so the suite boots once per
...
describe('integrator-express e2e', { skip: shouldSkip ? skipReason : false }, () => {
  ...
  function buildRouter(opts: {
    requireAuth?: boolean;
    resolveWorkspace?: Parameters<typeof createQelosMiddleware>[0]['resolveWorkspace'];
  }) {
    ...
    router.get('/secret', requireUser((_req, res) => res.json({ ok: true })));
  ...
  it('SDK entity CRUD via the integrator works end-to-end', async () => {
    const title = `e2e-express-${Date.now()}`;
